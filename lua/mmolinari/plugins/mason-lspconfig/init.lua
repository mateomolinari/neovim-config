return {
  ...
  config = function()
    ...
    local default_config = require('mmolinari.lsp.providers.defaults')
    ...
    local start_server = function(server)
      ...
      -- set up default mmolinari options
      if server == 'tsserver' then
        opts = u.merge(opts, require('mmolinari.lsp.providers.tsserver'))
      elseif server == 'jsonls' then
        opts = u.merge(opts, require('mmolinari.lsp.providers.jsonls'))
      elseif server == 'pyright' then
        opts = u.merge(opts, require('mmolinari.lsp.providers.pyright'))
      elseif server == 'lua_ls' then
        opts = u.merge(opts, require('mmolinari.lsp.providers.lua_ls'))
      end
