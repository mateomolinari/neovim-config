...
return {
  ...
  config = function()
    require('mmolinari.plugins.telescope.config')
  end,
  init = function()
    -- normal mappings
    ...
    map('n', '<leader>ff', '', {
      callback = require('mmolinari.plugins.telescope.utils').project_files,
      desc = 'Find file',
    })
  ...
  keys = {
    {
      '<leader>ff',
      '<cmd>lua require("mmolinari.plugins.telescope.utils").project_files()<cr>',
